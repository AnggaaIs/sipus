@if ($paginator->hasPages())
    <nav role="navigation" aria-label="Navigasi halaman" class="space-y-4">
        <div class="flex justify-center sm:hidden">
            <div class="inline-flex items-center gap-1 rounded-[calc(var(--radius)+0.125rem)] border border-[var(--border)] bg-[var(--card)] p-1">
                @if ($paginator->onFirstPage())
                    <span class="inline-flex h-10 w-10 items-center justify-center rounded-[var(--radius)] text-[var(--muted-foreground)]">
                ...
                @foreach ($elements as $element)
                    @if (is_string($element))
                        <span class="inline-flex min-w-10 items-center justify-center px-2 text-sm font-medium text-[var(--muted-foreground)]">
                            {{ $element }}
                        </span>
                    @endif
                ...
                @if ($paginator->hasMorePages())
                    <button wire:click="nextPage('{{ $paginator->getPageName() }}')" wire:loading.attr="disabled" rel="next"
                        class="inline-flex h-10 w-10 items-center justify-center rounded-[var(--radius)] text-[var(--muted-foreground)] transition-colors duration-200 hover:bg-[var(--accent)] hover:text-[var(--foreground)]"
