...
Versi pada tabel diambil dari lock file dan instalasi proyek saat ini.
...
| `livewire/livewire` | UI reaktif | Katalog publik dan komponen Filament berjalan tanpa frontend SPA terpisah | 4.3.1 | Constraint `*` di composer.json terlalu longgar dan sebaiknya dikunci |
...
| `spatie/laravel-permission` | Role dan permission | Disiapkan untuk otorisasi granular | 7.4.2 | Saat ini role utama masih disimpan langsung pada kolom `users.role` |
...
| `laravel/pao` | Output test untuk agent | 1.1.1 | Pada sebagian lingkungan Windows perlu `PAO_DISABLE=1` |
...
| Package | Fungsi | Versi terpasang | Risiko/Perhatian |
| --- | --- | --- | --- |
| `vite` | Build tool frontend | 8.0.16 | Memerlukan Node.js yang kompatibel |
| `laravel-vite-plugin` | Integrasi Laravel-Vite | 3.1.0 | Build gagal jika entry point tidak sesuai |
| `tailwindcss` | Utility-first CSS | 4.3.1 | Perubahan mayor berbeda dari konfigurasi Tailwind 3 |
| `@tailwindcss/vite` | Plugin Tailwind untuk Vite | 4.3.1 | Harus selaras dengan Tailwind |
| `@tailwindcss/forms` | Normalisasi komponen form | 0.5.11 | Style dapat memengaruhi komponen pihak ketiga |
| `@tailwindcss/typography` | Style konten tipografi | 0.5.20 | Tambahan CSS jika digunakan luas |
