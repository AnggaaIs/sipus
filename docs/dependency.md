...
Versi pada tabel diambil dari lock file dan instalasi proyek per 13 Juni 2026.

## Dependency Backend

| Package | Fungsi | Alasan digunakan | Versi | Risiko/Perhatian |
| --- | --- | --- | --- | --- |
| `laravel/framework` | Framework aplikasi | Routing, Eloquent, validasi, auth, queue, dan fondasi aplikasi | 13.9.0 | Upgrade mayor dapat mengubah API framework |
| `filament/filament` | Panel admin/user | Mempercepat pembuatan CRUD dan dashboard | 5.6.3 | Resource dan schema perlu mengikuti API Filament 5 |
| `livewire/livewire` | UI reaktif | Katalog dapat mencari dan memfilter tanpa SPA terpisah | 4.3.0 | Constraint `*` terlalu longgar dan sebaiknya dikunci |
...
| --- | --- | --- | --- |
| `pestphp/pest` | Framework test | 4.7.0 | Test database memerlukan `pdo_sqlite` |
...
| `laravel/boost` | Tool pengembangan Laravel | 2.4.6 | Hanya diperlukan di development |
| `laravel/pail` | Pembaca log Laravel | 1.2.6 | Jangan mengekspos log sensitif |
...
| `concurrently` | Menjalankan beberapa proses dev | `^9.0.1` | Digunakan lokal, bukan production |
...
- Filament mengurangi jumlah kode CRUD tetapi meningkatkan keterikatan pada
  API panel builder.
- Livewire membuat katalog reaktif tanpa API frontend khusus.
